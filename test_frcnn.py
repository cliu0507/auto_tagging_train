...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
	try:
		...
		if float(img.shape[0])/img.shape[1] >=2 or float(img.shape[0])/img.shape[1] <=0.5:
			continue
		...
		test = np.transpose(Y1[0], (2, 0, 1)) # just for pycharm debug usgae
		test_file = filepath # just for pycharm debug usgae
